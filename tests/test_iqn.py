...
from rejax import IQNConfig, train_iqn
...
def _train(env):
    config = IQNConfig(**ARGS)
    return train_iqn(config, jax.random.PRNGKey(0), env=env)
...
def _make_act(state, config=None):
    from rejax.algos.iqn import _make_act

    cfg = config or IQNConfig(**ARGS)
...
def test_env1():
    ...
    value = q_network.q(jax.numpy.array([0]), RNG)
